const grid = document.getElementById('dataBody');
...
const form = {
  ...
  credit: document.getElementById('inputCredit'),
  debit: document.getElementById('inputDebit'),
...
const storageKey = 'datagrid_crud_items';
function getData() {
...
function refreshGrid() {
  const data = getData().slice().sort((a, b) => new Date(a.date) - new Date(b.date));
  let balance = 0;
  let totalCredit = 0;
  let totalDebit = 0;
  grid.innerHTML = '';
  ...
  data.forEach((item, idx) => {
    ...
    grid.appendChild(renderRow(item, idx));
  });

  footerCredit.textContent = totalCredit.toFixed(2);
  footerDebit.textContent = totalDebit.toFixed(2);
